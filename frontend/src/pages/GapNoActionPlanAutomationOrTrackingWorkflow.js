...
/**
 * Gap/CFS Feature Page: No action-plan automation or tracking workflow.
 * Slug: no-action-plan-automation-or-tracking-workflow
 * Backend: /api/gap-no-action-plan-automation-or-tracking-workflow
 */
export default function NoActionPlanAutomationOrTrackingWorkflowPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-action-plan-automation-or-tracking-workflow', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>No action-plan automation or tracking workflow.</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>No action-plan automation or tracking workflow.</p>
