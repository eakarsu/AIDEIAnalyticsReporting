...
/**
 * Gap/CFS Feature Page: Bias mitigation playbook
 * Slug: bias-mitigation-playbook
 * Backend: /api/cf-bias-mitigation-playbook
 */
export default function BiasMitigationPlaybookPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/cf-bias-mitigation-playbook', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>Bias mitigation playbook</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>Generate evidence-based interventions (training, policy, process changes) to address identified biases.</p>
