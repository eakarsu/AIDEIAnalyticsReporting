/**
 * Custom Feature: peer-benchmark
 * Peer DEI benchmarking
 *
 * POST /api/ai/peer-benchmark
 * Auth required. Generated as part of Custom Feature Suggestions scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_PEER_BENCHMARK_KEY
...
const FEATURE_KEY = process.env.FEATURE_PEER_BENCHMARK_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Peer DEI benchmarking.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIDEIAnalyticsReporting - peer-benchmark',
    },
    body: JSON.stringify({
...
router.post('/peer-benchmark', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_PEER_BENCHMARK_KEY');
    const ai = await callLLM({ feature: 'peer-benchmark', goal: 'Peer DEI benchmarking', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['peer-benchmark', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[peer-benchmark] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'peer-benchmark',
    ...
    console.error('[peer-benchmark] error:', err.message);
    return res.status(err.statusCode || 500).json({ error: err.message || 'Internal error' });
  }
});

router.get('/peer-benchmark/health', (req, res) => {
  res.json({
    feature: 'peer-benchmark',
    endpoint: '/api/ai/peer-benchmark',
