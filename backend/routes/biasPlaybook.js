/**
 * Custom Feature: bias-playbook
 * Bias mitigation playbook
 *
 * POST /api/ai/bias-playbook
 * Auth required. Generated as part of Custom Feature Suggestions scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_BIAS_PLAYBOOK_KEY
...
const FEATURE_KEY = process.env.FEATURE_BIAS_PLAYBOOK_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Bias mitigation playbook.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIDEIAnalyticsReporting - bias-playbook',
    },
    body: JSON.stringify({
...
router.post('/bias-playbook', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_BIAS_PLAYBOOK_KEY');
    const ai = await callLLM({ feature: 'bias-playbook', goal: 'Bias mitigation playbook', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['bias-playbook', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[bias-playbook] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'bias-playbook',
      endpoint: '/api/ai/bias-playbook',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[bias-playbook] error:', err.message);
    return res.status(err.statusCode || 500).json({ error: err.message || 'Internal error' });
  }
});

router.get('/bias-playbook/health', (req, res) => {
  res.json({
    feature: 'bias-playbook',
    endpoint: '/api/ai/bias-playbook',
